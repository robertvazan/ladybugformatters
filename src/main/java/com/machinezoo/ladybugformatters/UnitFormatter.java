...
public interface UnitFormatter extends DoubleFormatter {
    String unit();
    char[] BIG_PREFIXES = "KMGTP".toCharArray();
    char[] SMALL_PREFIXES = "mμnp".toCharArray();
    @Override default String plain(double value) {
        if (value < 0)
            return "-" + plain(-value);
        if (value >= 1) {
    ...
    @Override default String detail(double value) {
        if (value < 0)
            return "-" + detail(-value);
        var format = NumberFormat.getInstance();
        format.setMinimumFractionDigits(0);
    ...
    @Override default DomContent format(double value) {
